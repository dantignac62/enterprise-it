...
| `RevertCISL1.Tests.ps1` | `Remove-HardenedRegistryValue`, `Restore-ServiceStartType` (from `Remove-CISL1Hardening.ps1`) | The revert helpers, registry/service cmdlets mocked: value removal vs no-op when absent, service start-type restore vs idempotent skip, `-WhatIf` honored, error-not-throw. |
